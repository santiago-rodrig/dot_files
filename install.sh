...
function resetColors () {
  echo -en "$RESET_INVERTED_COLORS$RESET_COLORS"
  echo -en "$RESET_INVERTED_COLORS$RESET_COLORS"
}
...
function beginInfo () {
  echo -en "$LIGHT_CYAN$1"
  resetColors
  echo -e ": $INVERTED_COLORS"
}
...
for CATEGORY in 'colors' 'editing' 'git' 'html' 'markdown' 'navigation' \
  'ruby' 'status-bar' 'textobjects'; do
